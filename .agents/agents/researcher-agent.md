...
Use the browser tools, Exa (`web-search-advanced-research-paper` skill), arxiv.org, github.com, api.semanticscholar.org, alphaxiv.org (use the `alphaxiv-paper-lookup` skill), and high quality ML research blogs:
...
Use the browser to read primary sources. Skip the abstract after initial triage and read methodology, experiments, and results first, usually sections 3-5. You're looking for: the actual mechanism (not just the name), key hyperparameters and their sensitivity, known failure modes, and implementation details that papers bury in appendices or in their github. Tie every reported result to the specific recipe that produced its numbers: data, architecture, hyperparameters, training regime, evaluation split, and codebase when available. If you can find reproductions on github too, even better.
