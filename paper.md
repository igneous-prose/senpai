...
CFD surrogates promise a practical acceleration layer for engineering design: inexpensive learned models can screen early-stage geometries as design candidates, explore diverse operating regimes, and focus high-fidelity CFD and physical testing for later decisions. The technology has advanced quickly: modern surrogates now handle larger meshes, irregular geometries, tighter accuracy requirements, and smaller training sets than were feasible only a few years ago [Justin_4]. In principle, this makes a hybrid workflow attractive, whereby ML surrogates complement established simulation pipelines.

In practice, adoption remains difficult for many small/mid-size engineering organizations. Training a useful CFD surrogate is not a routine application of standard ML: it requires choices about architecture, optimization, normalization, boundary conditions, symmetries, rollout stability, and benchmark-specific evaluation [Justin_1]. The best models, and usage thereof, are typically produced by teams that combine ML expertise with domain knowledge, for example aerodynamics or fluid mechanics [Justin_2]. Large enterprises can build such teams internally, but most engineering groups are already constrained by safety-critical design processes, shortening development cycles, and expanding technical requirements. SENPAI targets this gap: it provides an autonomous research harness that can explore, document, and refine problem-specific surrogate recipes while keeping human engineers in the review loop.
...
All agent trajectories were logged to W&B, enabling audits of common failure modes. During a high-activity 24-hour deployment window, SENPAI recorded 53,022 Claude model requests, split across 47,053 Opus 4.6 and 5,969 Sonnet 4.6 calls. In this window the system consumed 5.24B cache-inclusive tokens, of which 5.11B were prompt-cache reads and 132.1M were uncached work. The 57 student fleet dominated this token usage, with 5.05B cache-inclusive tokens (96.4%) versus 191.2M for the advisor (3.6%); students used 46,017 Opus / 3,932 Sonnet calls, while the advisor used 1,036 Opus / 2,037 Sonnet calls. 

`tail -f | grep epoch`, a warning
The most common failure mode observed was that  cache-read mass was primarily repeated long-running training-session state rather than generic prompt overhead. Across the window, training execution and monitoring accounted for 40,820 Claude model calls, or 77.0% of all Claude requests in the window, and 4.36B cache-read tokens, or 85.4% of all cache-read tokens. A more detailed audit showed that the dominant token driver was training supervision by the student fleet. Although students made only 615 explicit Claude Code `Monitor` tool calls, these monitors registered narrow shell watchers over training logs, typically filtering for epoch, validation, best-checkpoint, and error signals. The resulting event stream produced 28,247 monitor notifications and 28,246 direct model responses. These responses accounted for 3.25B cache-inclusive tokens; each response reloaded a median 110k cached context tokens but introduced only a median 347 uncached tokens. Thus, SENPAI’s monitoring loop used narrow external checks, but each check was interpreted inside the full cached state of the corresponding long-running student session. While more spaced Monitor tool calls would have reduced the frequency of reporting, this still wouldn’t have eliminated the main issue that results from Monitor tool calls are fed back to the main agent context, as opposed to a low-context sub-agent that could choose to surface the monitor event to the main agent.

Other aspects of the cache include active PR assignments, role/tool context, accumulated code/log outputs, W&B metric histories, prior assistant decisions. The uncached side was much smaller and mostly consisted of newly appended monitor events and command outputs, such as fresh epoch metrics, W&B/status query results, GitHub PR/comment state, cache writes after context growth or compaction, and generated summaries/actions. 

Because logging and observability is core to SENPAI, the system was able to be improved, with the codebase improved based on the learnings from these agent trajectories. 

240 conversation compactions were also recorded during the window with each student having an average of 2.4 compactions between the main thread and sub-agents, and the advisor having 7 top-level compactions and 94 sub-agent  compactions, most commonly for PR-assignment sub-agents.
...
0. *Research.* The Advisor may decide to perform research to decide on what to try next.
1. *Proposal.* The Advisor drafts a PR containing a hypothesis, the exact training code guidelines to run against the current baseline, and the metrics to improve.
2. *Assignment.* A PR label names the target Student; the Student's outer loop polls for its PRs.
3. *Implementation and training.* The Student checks out the branch, implements the changes to the shared training entrypoint, and launches training experiments under a structured W&B run name.
4. *Reporting.* On completion the Student reports back by posting a PR comment with a summary of the experiment results and changes the PR status from wip to review.
5. *Review.* The Advisor notices that PR status has changed and enter review mode, checks validation metric (§2.3) and decides: squash-merge (accept), comment-and-reassign (revise), or close (reject).
6. *Baseline advance.* A squash merge updates the current baseline on the Advisor branch, against which every subsequent PR is compared.
...
2.1 Pull requests and W&B runs as first-class state

Every hypothesis is a pull request. The PR body holds the hypothesis, the experiment instructions, and the baseline metrics to beat; the thread carries the Student's results comment and any Advisor review. W&B carries the quantitative side: each run uses a `<student>/<description>` name and the run ID is posted back in the Student comment, joining each PR to its runs. The two substrates together form the **experiment ledger** — human-inspectable through the ordinary GitHub and W&B UIs, and machine-queryable through the same CLIs human researchers use. Pod restarts, context compactions, and fresh-container boots discard in-session state; the next cycle recovers by re-reading the ledger rather than reconstructing from memory. In contrast to AgentRxiv [Schmidgall et al., 2025], which shares papers between agents, SENPAI shares experiments within an existing code-review substrate.

The Advisor also keeps a small set of focus-aid markdown files on its long-running branch (research-state summary, baseline, experiments log, dataset notes), regenerated each cycle. These are not authoritative state: an earlier iteration of the harness treated them as durable, which produced silent drift when summaries disagreed with the ledger (§4.3). An anonymised end-to-end PR transcript is reproduced in Appendix E.

2.2 The harness loop and observability daemons

SENPAI deploys as a small set of Kubernetes workloads: one Advisor CPU pod, N Student GPU pods, and a shared persistent volume for datasets and checkpoints. The harness image is stable across problems; the *problem-package* repository is swapped in per programme (bring-your-own-repo), so every agent commit, branch, and PR lives in a self-contained target repository that a reviewer can inspect without the harness. A single configuration file selects the active problem package; a launch helper renders the Kubernetes manifests and applies them.

**The outer loop is a shell wrapper, not an LLM.** The Advisor entrypoint wraps Claude Code in a `while true` iteration, in the spirit of Huntley's "ralph loop" pattern. We chose a bespoke wrapper over adopting ralph-loop directly for two reasons. First, we wanted *out-of-LLM triage* before every tick, so that a quiescent programme spends no LLM tokens. Second, the entrypoint doubles as the place where substrate-specific fixups live — re-materialising the agent role instructions (which the agent's own `git` operations occasionally clobber with the developer copy), injecting the triage summary into the next prompt, and recovering across the agent's session exits by resuming the prior session on iterations ≥ 2. Advisor ticks run every 10 min, Student ticks every 60 s; both cadences are economical only because of this gate. In outline:

```
# Advisor outer loop (shell wrapper, no LLM call in the gate)
...
while True:
    ...
    triage = render_triage(review_ready, human_issues, idle_students)
    if iteration == 1:
        exit_code = run_agent(role_prompt + triage)              # fresh session
    else:
        exit_code = run_agent(heartbeat + triage, resume=True)   # resume prior session
...
**Observability daemons.** Two write-once logging services run alongside every agent pod:

- *Session transcript service (internal tool).* Streams every tool call, LLM message, and context compaction from each Claude Code session to a central transcript store. No agent reads its own transcripts; they underpin the token accounting in §3 and the failure-mode coding in §4.3, and give reviewers post-hoc visibility into *why* each decision was taken.
- *Weave CC plugin* is a Claude Code plugin installed into every Advisor and Student pod as a daemon that streams the same session as W&B Weave traces. Tool calls, sub-agent invocations, and token spend become queryable alongside the W&B runs the agents launch, closing the loop between the agent's *decision process* and the *training result* it produced.

Between the PR ledger (what was decided), the W&B run (what was trained), the Weave trace (why the agent picked it), and the raw session transcript (session tape), the system is observable at **four independent layers**. This is what the "observability-first" framing in §1 refers to, and it is what makes human course-correction and post-hoc audit cheap.

2.3 Human-in-the-loop

Humans steer SENPAI through three channels, all writing into the substrate the agents already read. *PR review* is direct: any human comment on a draft PR is picked up by the Advisor in the next review cycle, and any human review on a `status:review` PR is honoured before merge. *Issue-channel steering* surfaces human-authored GitHub Issues at the top of each Advisor iteration; `urgent`-tagged issues are also routed to Students, where they can terminate an in-flight training run. *Fleet control* through the deployment entrypoint supports pause, scale, and redirect of the Advisor and Student pools from the host shell. During multi-day unattended sessions human actions were limited to occasional course corrections (roughly one per day) and pod restarts; no training-config edits and no human merges occurred during the documented unattended window (Appendix F).
...
A recurring reviewer question about agentic research systems is *where do they break.* We treat this as a first-class contribution and document three systematic failure modes, plus one cognitive failure mode observed in the kagent case study (§4.2). All four are released in annotated form alongside the harness so future systems can be scored against the same categories.

*Coding protocol.* Failure modes were identified by sampling 241 agent sessions (Advisor and Student) spanning ~242 million tokens of session transcripts, cross-indexed against the PR ledger. A session was annotated when at least one of the following was observable: (i) a tool call that contradicted the PR substrate's ground truth, (ii) a summary document that disagreed with the PR/W&B ledger at the moment of reading, (iii) a training run that did not produce a recoverable checkpoint, or (iv) a sequence of ≥ 5 iterations in which the agent's own stated diagnosis was consistent but its fix did not change the relevant metric. Each session was coded independently, then disagreements reconciled. The annotated session sample and the coding rubric are released in Appendix C.

**Tool misuse (no decrease under prompt hardening).** Across three successive rounds of Advisor-prompt tightening — each round added explicit guards against a specific misuse pattern observed in the prior round — the per-session tool-misuse rate did not trend down (3.52 → 4.68 → 4.58 per session). Representative misuses: calling `gh pr review --approve` against a PR the Advisor had not actually reviewed; invoking `wandb` queries with a stale run filter; marking a PR `status:merged` via label without the corresponding git merge. Prompt hardening shifted *which* misuse occurred without reducing the overall rate. The operational implication is concrete: agentic research harnesses should treat privileged operations as idempotent, verifiable skill calls rather than relying on prompt-level guardrails. SENPAI's `senpai-gh` helper and the `merge-winner` skill reflect this lesson.

**Derived summary-state drift.** Early iterations of the harness leaned on Advisor-maintained markdown summaries (`CURRENT_RESEARCH_STATE.md`, `BASELINE.md`) as durable programme state. In 14 observed sessions the summary disagreed with the PR ledger at read time — typically because a merged PR had updated the baseline but the summary document had not been regenerated. In every such case the agent's next hypothesis was grounded in the stale summary. The fix was not "write better summaries" but "stop using summaries as state": skills now read PR labels and W&B metadata directly and the summary documents are regenerated each cycle as focus aids only (§2.2).

**Contract-invisible infrastructure (session mortality and scoring races).** Short Student sessions are robust to context compaction by design, but are *not* robust to mid-training pod eviction if the training loop has not written an evaluation-quality checkpoint. In the early programme, 17 Student sessions terminated after a productive training run but before the best-checkpoint writer fired, leaving the PR with loss curves but no evaluable artifact. The fix was infrastructural: PR #3029 made best-checkpoint saving a mandatory training-loop responsibility, after which the failure mode disappeared. The kagent TandemFoil comparator (§4.2, Appendix G.2) surfaced a sibling instance of the same class: a scorer-race in the organiser's polling loop treated mid-write prediction directories as terminally `"incomplete"` and never re-scored them, invisibly silencing two agents for roughly an hour each. In both cases the underlying pattern — *agent correctness relies on an infrastructure contract the agent cannot inspect* — is the failure, and the fix is always at the infrastructure layer rather than the prompt.

**Correct diagnosis, no escape (cognitive, §4.2).** The kagent comparator documents an agent (*frieren*) that diagnosed its own plateau correctly — *"per-point MLPs cannot predict turbulent residuals from local input alone"* (Appendix G.2) — and still failed to leave the basin across eight subsequent iterations. This is neither tool misuse nor state drift; the tools worked and the diagnosis was accurate. It is approach-space inertia: once a Student has sunk iterations into a parameterisation, its subsequent edits perturb that parameterisation rather than replace it. SENPAI's Advisor role and plateau-escalation prompt partially address this, but as a prompt-level guideline rather than enforced logic (§2.3); the mode is not eliminated and a stronger escalation substrate is open work.

None of the four failure modes close under stronger prompts. Each is handled — partially — by a substrate- or infrastructure-level change: idempotent skills for tool misuse, ledger-is-ground-truth for derived-state drift, a checkpoint contract in the training loop for session mortality, and (partially) a plateau-escalation protocol for cognitive inertia. The taxonomy's practical implication is that agentic research systems should be *designed for* these failure modes, not expected to avoid them through prompting alone.
4.3 Limitations
...
- *Frieren* (iter93, inference from askeladd's W&B config): *"bs=2 + no-subsample + warm-start = BREAKTHROUGH. Subsampling was the root cause of my re_rand weakness — dropping 60% of volume nodes left the model unable to learn Re-dependent field structure. With no subsampling the model sees the full 240K-node grid. Askeladd's edge was entirely this config difference."* [[journal L39–44]](https://github.com/tcapelle/kagent/blob/cdfa5d78e04aa2b6977e3d5ab8a8f4d1c0dfcc91/tandemfoil-competition/kaggler/EXPERIMENT_JOURNAL.md#L39-L44) Scored 35.27 (was 52); `re_rand` dropped 73 → 33.43. Sixty further commits polished this into the 6-way ensemble at 34.41.
- *Alphonse* (v11, direct hypothesis about slice-weight distribution shift): *"The PhysicsAttention slice weights are computed on a different density of nodes between train and val … the hypothesis that subsampling 'just acts like a regulariser' was wrong in this domain; it creates a real train/eval distribution gap for attention models that pool over the node set."* [[journal L72–76]](https://github.com/tcapelle/kagent/blob/d145d312660459324e386042eaa69a14f3a48d48/tandemfoil-competition/kaggler/EXPERIMENT_JOURNAL.md#L72-L76) 22% single-run gain; `val_re_rand` 2.70 → 1.25.
- *Edward* (v13, inferred from askeladd's W&B runs): *"Sub40k was great for pre-training, but fine-tuning at full mesh preserves fine surface detail that's essential for pressure MAE … 9 epochs at full mesh beats 35 at sub40k in this regime."* [[journal L32]](https://github.com/tcapelle/kagent/blob/a76f87a17574d9490d868edad359460f22a1465c/tandemfoil-competition/kaggler/EXPERIMENT_JOURNAL.md#L32) 15.7% drop in one run, lifting edward from rank 7 to rank 3.

The three agents who did not find the unlock stalled. *Thorfinn* documented the plateau precisely (*"clean logarithmic decay; we hit the architecture's capacity floor"* [[journal L37]](https://github.com/tcapelle/kagent/blob/aa76905ad4467a95bcd914b588ef077802ce6c88/tandemfoil-competition/kaggler/EXPERIMENT_JOURNAL.md#L37)) without testing whether the floor was the subsampling, and fell from rank 3 to rank 6. *Tanjiro* never tried full-mesh. *Nezuko* went in the opposite direction, tuning `surf_weight` down from 10 → 1.5; her own earlier-formulated rule — *"compute-per-epoch is the binding constraint: any change that slows each batch needs a matching epochs adjustment and usually nets negative"* [[journal L151]](https://github.com/tcapelle/kagent/blob/4e69f0371b01da656cd1dc89a3295cc0be50e3f9/tandemfoil-competition/kaggler/EXPERIMENT_JOURNAL.md#L151) — kept her away from a change (full mesh) that violated it.

**Two robust secondary findings.**

1. *Same-lineage ensembles regress.* Every agent that ensembled checkpoints from a single warm-start chain reported regression, and several explicitly formulated the rule — *"members have to be individually close in quality, or strongly decorrelated"* (alphonse [[journal L110]](https://github.com/tcapelle/kagent/blob/d145d312660459324e386042eaa69a14f3a48d48/tandemfoil-competition/kaggler/EXPERIMENT_JOURNAL.md#L110)); *"v1 is a strictly-worse ancestor of v2"* (askeladd [[journal L125]](https://github.com/tcapelle/kagent/blob/054c788eb583dd052b87ca3a4dabcbcf75efa672/tandemfoil-competition/kaggler/EXPERIMENT_JOURNAL.md#L125)); *"only ensemble strong models"* (frieren [[journal L99]](https://github.com/tcapelle/kagent/blob/cdfa5d78e04aa2b6977e3d5ab8a8f4d1c0dfcc91/tandemfoil-competition/kaggler/EXPERIMENT_JOURNAL.md#L99)). Frieren eventually made ensembling pay, but only after introducing architectural diversity (`slice=64` vs `slice=128` chains).
