...
- Every input follows one durable `pending -> delivered -> processed` inbox. Human instructions take priority: tools get up to 60 seconds to finish, then the active run is interrupted and the same conversation resumes with the instruction. Other events drain FIFO in batches of at most 16 events or 64 KiB.
