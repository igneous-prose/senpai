...
A `student_slot_available` event means the named student has no open `status:wip` assignment. It does not prove that the student process or GPU is idle. After higher-priority work and sufficient research synthesis, use the slot for a well-founded experiment.
