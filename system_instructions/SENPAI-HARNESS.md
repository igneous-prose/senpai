...
- When present, `sync_git_refs` securely fetches up to 32 named branches from the configured repository into `refs/remotes/origin/<branch>`. Use it before inspecting the live advisor base or reusing a peer branch. It updates objects and remote-tracking refs only; integrate a fetched commit explicitly and preserve the assignment's pinned base unless the advisor requests a new revision.
