...
\begin{figure*}[t]
  ...
  \caption{CFD surrogate problems used by SENPAI, shown as geometry-and-field examples rather than benchmark results. (a) DrivAerML is a 3D automotive aerodynamics task with millions of surface and volume sample points; the panel shows pressure-colored surface samples together with downstream wake nodes. (b) AirfRANS contains families of 2D airfoil cross-sections. (c) TandemFoilSet contains paired-foil fields, here shown for race-car and cruise configurations with surface nodes overlaid on field samples.}
...
\paragraph{DrivAerML.}
We use the public processed-case split, 400/34/50 train/validation/test, and report surface-pressure, vector wall-shear-stress, and volume-pressure relative-$L_2$, computed per case on unnormalized predictions and then averaged over test cases.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    SENPAI, single model & 3.88 & 7.06 & 10.76 \\
...
The PR ledger shows the path: lower learning rates near \texttt{1e-4}, gradient clipping, EMA, warmup, and schedule-length tuning.
...
This section contains verbatim snapshots of the prompt and contract artifacts summarized in \Cref{app:agent-instructions}. The files are vendored into \texttt{papers/appendix\_sources/}; external artifacts are snapshots of the public GitHub URLs listed in \Cref{tab:instruction-source-map}.
