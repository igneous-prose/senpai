...
SENPAI's central design choice is to ground the research loop in external systems of record that ML teams already inspect: pull requests and git history for hypotheses, code changes, review, and provenance \citep{githubdocs_pullrequests,chacon2014progit}; and a hosted experiment tracker for configurations, metrics, checkpoints, system telemetry, and agent traces \citep{biewald2020wandb,wandbdocs_experiments}. Our implementation uses GitHub and Weights \& Biases,\footnote{Weights \& Biases was the hosted experiment tracker used in the reported experiments.} but the harness design is not specific to either service. Each hypothesis, code diff, training result, review decision, failed run, and baseline update becomes both machine-queryable and research-reviewable through these existing tools.
...
Across the research programme, SENPAI produced over 3,000 PRs, 11,000 experiment-tracker runs, and operated with up to 59 concurrent Student agents, turning a multi-week semi-autonomous research programme into a durable experiment ledger.
...
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=2pt,parsep=0pt]
  ...
  \item \textbf{CFD recipe search.} Starting from Transolver-style baselines, SENPAI discovers improved recipes across AirfRANS, DrivAerML, and TandemFoilSet, with strongest gains on surface quantities.
...
\subsection{Experiment-Logging Ablation}
...
For the hosted-logging Willow arm, \cref{fig:willow-sankey} summarizes how hypotheses moved from Advisor branches through experiment families to final PR outcomes.

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.92\textwidth]{icml2026/figures/willow_sankey/willow_simplified_sankey.pdf}
  \caption{Experiment-ledger flow for the hosted-logging Willow ablation arm on TandemFoilSet-Balanced. Five Advisor branches (A1--A5) are routed through hypothesis families and into final PR outcomes; ribbon width is proportional to PR count. The figure compresses 167 GitHub-mediated experiments into a researcher-readable audit map, showing where autonomous search effort was allocated and how it resolved into merged, closed, or still-open work.}
  \label{fig:willow-sankey}
\end{figure*}

\begin{table}[t]
  \caption{Experiment-logging ablation on TandemFoilSet-Balanced. Surface-pressure MAE; lower is better.}
...
For double-blind review, non-anonymous links to the harness repository and hosted run ledger are withheld from this version and will be included in the camera-ready version. The CFD datasets used in the experiments are DrivAerML, AirfRANS, and TandemFoilSet; access and redistribution follow the licenses and terms provided by the original dataset authors.
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}p{0.22\textwidth}p{0.31\textwidth}p{0.39\textwidth}@{}}
    ...
    \midrule
    Advisor prompt & \url{system_instructions/CLAUDE-ADVISOR.md} & Defines the no-GPU research lead: survey PRs, review result comments and W\&B runs, merge validated winners, close dead ends, and assign one-hypothesis PRs to idle Students. \\
    Student prompt & \url{system_instructions/CLAUDE-STUDENT.md} & Defines the GPU worker: claim only assigned PRs, edit the task training file, run bounded experiments, report exact commands, metrics, W\&B IDs, and structured terminal results. \\
    Researcher sub-agent & \url{.claude/agents/researcher-agent.md} & Defines a literature and experiment-history specialist used to produce mechanism-grounded experiment proposals and research-state updates. \\
    Problem contract & \texttt{DrivAerML/program.md} & Example task contract specifying mission, data, editable files, metric keys, checkpoint selection, telemetry, and paper-facing target values. \\
    TandemFoil2 contract & \url{target/tandemfoil2/program.md} & Active local problem contract for the \texttt{kagent}-seeded TandemFoil2 package, including split names, edit boundaries, and the \texttt{val/l2\_error} leaderboard metric. \\
    Flat comparator prompt & \texttt{KAGGLER\_AGENT.md} & Defines the \texttt{kagent} loop: watch leaderboard and W\&B, edit local training/prediction code, keep only useful code changes, always journal outcomes, and continue until interrupted. \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[h]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Setting & Value \\
    \midrule
    Source & PR \#741 in the experiment ledger \\
    W\&B runs & train \texttt{lszc4ri7}, eval \texttt{1tal40wr} \\
    Training budget & 50 epochs; validation-selected checkpoint \\
    ...
    Point sampling & Train: 65,536 surface / 65,000 volume; eval: 40,000 surface / 65,000 volume \\
    Augmentation & Training-only $y$-plane reflection with lateral coordinate, normal, and $\tau_y$ sign flips \\
    Loss & Normalized MSE with surface/volume weights 1.0/1.0 \\
    \bottomrule
  \end{tabular}
\end{table}
...
As an additional stress test, sixteen Claude Opus 4.6 \texttt{kagent} agents were pointed at the GRaM ICLR 2026 open competition from 2026-03-29 to 2026-03-30. The winning cohort member reached 0.8526 mean L2 against a roughly 1.75 reference-MLP baseline, and the strongest checkpoint was packaged and submitted to the upstream competition as a live pull request. This run predates the later experiment-journal convention, so its evidence is primarily leaderboard and commit based.
