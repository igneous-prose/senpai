...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrr@{}}
    ...
    Method & $p_s$ & $\tau$ & $p_v$ \\
    \midrule
    SENPAI H147 (ours) & \best{3.5634} & 6.5409 & \best{3.4014} \\
...
\paragraph{Recipe.}
The selected single-model recipe SENPAI found used the original Transolver backbone but scaled it to 6 layers, 512 hidden units, and 128 slices, replaced fixed sinusoidal coordinates with a learnable Fourier positional embedding initialized across multiple spatial scales \citep{li2021learnable,schenck2025learningropes}, and trained with Lion \citep{chen2023symbolic}, EMA, GradNorm adaptive loss balancing \citep{chen2018gradnorm}, curvature attention bias, and $y$-symmetry augmentation. It trained with 8-GPU DDP for an effective batch size of 8 under a 30-epoch budget, with the best EMA checkpoint selected at epoch 25. The key final ablation was a Lion optimizer setting, $\beta_1=0.95,\beta_2=0.98$, that reproduced the early H138 trajectory while otherwise retaining the preceding best H39 recipe from the campaign. A notable empirical finding from SENPAI was the application of this lightweight STRING-inspired encoding from computer vision and robotics use cases to CFD surrogate modelling.

\paragraph{Scope.}
We report only the selected single trained checkpoint: no ensembles, checkpoint interpolation, SLSQP or bias-correction postprocessing, cohort postprocessing, or test-time augmentation. This makes the pressure gains directly attributable to the trained H147 checkpoint while preserving the remaining wall-shear gap to the strongest public wall-shear references.
...
\paragraph{Result.}
To demonstrate SENPAI's applicability beyond CFD research, we also ran SENPAI on Keller Jordan's modded-NanoGPT Track 3 optimization benchmark \citep{jordan2026moddednanogpttrack3}. Track 3 fixes the dataset, batch size, and architecture, and scores methods by the fewest optimizer steps needed to reach 3.28 validation loss on FineWeb \citep{penedo2024fineweb,karpathy2024llmcgpt2}.

We ran five SENPAI trials to measure result consistency and understand the distribution of autonomous search outcomes.\footnote{Final step results: \href{https://github.com/morganmcg1/modded-nanogpt-senpai/pull/T1}{<T1>/<PR1>}; \href{https://github.com/morganmcg1/modded-nanogpt-senpai/pull/T2}{<T2>/<PR2>}; \href{https://github.com/morganmcg1/modded-nanogpt-senpai/pull/T3}{<T3>/<PR3>}; \href{https://github.com/morganmcg1/modded-nanogpt-senpai/pull/T4}{<T4>/<PR4>}; \href{https://github.com/morganmcg1/modded-nanogpt-senpai/pull/T5}{<T5>/<PR5>}.} The core SENPAI system prompts were unchanged; only the repository-specific prompts were adapted to modded-NanoGPT. We also barred inspection or reuse of Prime Intellect's autonomous nanoGPT speedrun result \citep{primeintellect2026autonanogpt} to avoid biasing SENPAI with another autoresearch artifact.

Across the five trials, SENPAI ran 9,591 tracked experiments total using 9,772 training GPU-hours on NVIDIA RTX PRO 6000 Blackwell 96GB GPUs, or 1,954 GPU-hours per trial on average. For comparison, Prime Intellect's winning Claude Code search reached 2930 steps; the accounting in \Cref{app:modded-nanogpt-details} attributes roughly 2.6k of its reported 14k H200 GPU-hours to the Claude Code subset \citep{primeintellect2026autonanogpt}. Using the same peak-throughput estimate, one average SENPAI trial is approximately 3.5 zettaFLOP, versus about 9 zettaFLOP for the Claude Code subset.

SENPAI produced a candidate Track 3 result at 2925 steps, state-of-the-art as of publication, and pending official ratification. The result is recorded in PR~\#\texttt{<BEST\_RESULT\_PR>}.
...
The \href{https://github.com/wandb/senpai}{SENPAI harness} and \href{https://github.com/tcapelle/kagent}{\texttt{kagent} comparator} are released publicly. Public experiment ledgers are available through GitHub PR histories in \href{https://github.com/wandb/senpai}{\texttt{wandb/senpai}} and \href{https://github.com/morganmcg1/TandemFoilSet-Balanced}{\texttt{TandemFoilSet-Balanced}}, with W\&B run records under the projects listed in \Cref{app:wandb-projects}.
...
\begin{center}
  ...
  \begin{tabular}{@{}p{0.30\textwidth}p{0.62\textwidth}@{}}
    ...
    \midrule
    \href{https://wandb.ai/wandb-applied-ai-team/senpai-v1}{\texttt{senpai-v1}} & Primary SENPAI experiment-tracking project for the paper. \\
...
Track 3 of the modded-NanoGPT benchmark fixes the dataset, batch size, and model architecture so submissions primarily test optimizer and training-recipe changes. Valid submissions must use non-cherry-picked runs, include reproducible logs, avoid validation-loss-based per-run early stopping, and satisfy the benchmark precision rule $(3.28-\mu)\sqrt{n}\ge 0.004$ at a shared stopping point.
...
For the compute comparison in \Cref{sec:results}, we use NVIDIA's published dense BF16/FP16 tensor throughput for RTX PRO 6000 Blackwell and H200 GPUs \citep{nvidia2025rtxpro6000blackwell,nvidia2026h200}. This gives a rough hardware-peak upper bound of about 3.5 zettaFLOP for one average SENPAI trial. Prime Intellect reports roughly 14k H200 GPU-hours across its autonomous nanoGPT speedrun. In its released run archive, Claude Code accounts for 2,204 of 10,428 exported runs and 18.6\% of summed logged training seconds. Allocating the reported GPU-hours in proportion to that logged training-time share gives roughly 2.6k H200 GPU-hours, or about 9 zettaFLOP at dense H200 BF16/FP16 peak. These estimates use hardware peak throughput, not measured realized training FLOPs.
...
\begin{center}
  ...
  \begin{tabular}{@{}p{0.24\textwidth}p{0.66\textwidth}@{}}
    ...
    Result scope & Single trained checkpoint; no ensembles, checkpoint interpolation, SLSQP/bias correction, cohort postprocessing, or test-time augmentation \\
