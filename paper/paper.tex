...
\usepackage{latex/icml2026/algorithm}
\usepackage{latex/icml2026/algorithmic}
\usepackage{latex/icml2026/fancyhdr}
\usepackage[accepted]{latex/icml2026/icml2026}
...
\newcommand{\willowsankeyteaser}{%
  ...
  \begin{center}
    \begin{minipage}{0.70\textwidth}
      \centering
      \includegraphics[width=\linewidth]{figures/willow_simplified_sankey.png}
      \vspace{-0.17in}
...
\newcommand{\appendixsourcelink}[1]{\href{https://github.com/wandb/senpai/blob/main/paper/appendix_sources/#1}{\nolinkurl{#1}}}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{figures/senpai_architecture.png}
...
SENPAI follows three design choices: externalizing experiment state into existing systems of record, keeping the autonomous loop thin, and exposing the same ledger to both agents and researchers. The role prompts and source map are summarized in \Cref{app:agent-instructions}, the Advisor outer-loop pseudocode in \Cref{app:outer-loop}, and the problem-program contract and helper skills in \Cref{app:program-contracts,app:skill-catalogue}.
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/cfd_problem_gallery.pdf}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/drivaerml_surface_pressure_pareto.pdf}
...
\paragraph{Scale.}
The research programme created over 3,000 PRs and recorded over 11,000 experiment-tracker runs, with peak deployment of 59 Student agents and 347 concurrent training runs.
...
\paragraph{DrivAerML.}
We use the public processed-case split, 400/34/50 train/validation/test, and report surface-pressure, volume-pressure, volume-velocity, and vector wall-shear-stress relative-$L_2$, averaged per case on unnormalized predictions. Each raw DrivAerML case contains approximately 8.8M surface points and 160M volume points.
...
\paragraph{Result.}
On DrivAerML, SENPAI's best single trained checkpoint is \href{https://github.com/morganmcg1/DrivAerML/pull/1344}{H147 from PR~\#1344}, W\&B run \href{https://wandb.ai/wandb-applied-ai-team/senpai-v1-drivaerml-ddp8/runs/k6q4c3on}{\texttt{k6q4c3on}}. Starting from a Transolver-based implementation, it improves on the original Transolver for all four reported relative-$L_2$ metrics and gives the best single-model pressure results among the public references in \Cref{tab:drivaerml}: 3.6679\% surface pressure and 3.4004\% volume pressure, compared with 3.71\% and 5.72\% for Transolver-3. Its volume-velocity error is 5.0911\% and wall-shear error is 6.6676\%; both improve on Transolver, while volume velocity remains behind Transolver-3 and Transolver++ and wall shear remains behind Transolver-3 and Transolver++. The full recipe, run provenance, and per-axis wall-shear diagnostics are in \Cref{app:drivaerml-recipe}. \Cref{fig:drivaerml-surface-pareto} shows the semi-autonomous experiment trajectory for surface-pressure test error across earlier SENPAI campaigns.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrr@{}}
    \toprule
    Method & $p_s$ & $p_v$ & $u_v$ & $\tau$ \\
    \midrule
    SENPAI (ours) & \best{3.67} & \best{3.40} & 5.09 & 6.67 \\
    Transolver-3 \citep{zhou2026transolver3} & 3.71 & 5.72 & \best{4.14} & \best{5.85} \\
    AB-UPT \citep{alkin2025abupt} & 3.82 & 6.08 & 5.93 & 7.29 \\
    Transolver++ \citep{luo2025transolverplusplus} & 4.12 & 6.70 & 4.70 & 6.42 \\
    Transolver \citep{wu2024transolver} & 4.81 & 7.74 & 6.78 & 8.95 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\paragraph{Result.}
To demonstrate SENPAI's applicability beyond CFD research, we also ran SENPAI on Keller Jordan's modded-NanoGPT Track 3 optimization benchmark \citep{jordan2026moddednanogpttrack3}. SENPAI produced a Track 3 result at 2925 steps, which was competitive with the best publicly visible results in the repo at the time. The result is recorded in \href{https://github.com/morganmcg1/modded-nanogpt-senpai/pull/737}{morganmcg1/modded-nanogpt-senpai PR~\#737}. We estimate that one average SENPAI trial corresponds to roughly 3.5 zettaFLOP of hardware-peak training compute; see \Cref{app:modded-nanogpt-details} for the training recipe and further details.
...
The strongest empirical results are on surface quantities, primary signals in many aerodynamic design workflows. Volume-field prediction remains less developed on AirfRANS, where SENPAI improves surface MSE while remaining behind the strongest published volume-MSE method; on DrivAerML, the selected checkpoint improves both pressure fields and volume velocity over the original Transolver while wall-shear remains behind the strongest public reference.
...
\bibliographystyle{latex/icml2026/icml2026}
\bibliography{paper}
...
\begin{center}
  ...
  \begin{tabular}{@{}p{0.24\textwidth}p{0.66\textwidth}@{}}
    ...
    Reproduction report & \href{https://wandb.ai/wandb-applied-ai-team/senpai-v1-drivaerml-ddp8/reports/-2026-06-22-DrivAerML-H147-Repro-vs-Volume-Velocity-Targets--VmlldzoxNzI3MTE5MQ}{W\&B report documenting a three-run reproduction} \\
    Test metrics & Surface pressure 3.6679\%; volume pressure 3.4004\%; volume velocity 5.0911\%; wall shear 6.6676\% \\
...
The 2925-step result is recorded in \href{https://github.com/morganmcg1/modded-nanogpt-senpai/pull/737}{morganmcg1/modded-nanogpt-senpai PR~\#737}. The selected recipe adds a cooldown-aware Polyak EMA $\beta$ ramp to the Track 3 training script. The EMA uses base $\beta=0.95$ and starts after \texttt{ema\_warmup\_steps=2250}; during cooldown it couples the averaging window to the body-Muon learning-rate schedule as $\beta_t=\beta_{\mathrm{base}}+(\beta_{\mathrm{target}}-\beta_{\mathrm{base}})(1-\mathrm{lr\_mult}_t)$. The winning arm used $\beta_{\mathrm{target}}=0.99$, so the EMA window grows as the learning rate decays and live-parameter drift shrinks. A more aggressive $\beta_{\mathrm{target}}=0.999$ arm was also run and was not selected.

PR~\#737 reports two confirmation seeds for the selected $\beta_{\mathrm{target}}=0.99$ arm, W\&B runs \texttt{rdbmnzpc} and \texttt{32r3isz5}. Both reached \texttt{speedrun/final\_first\_step\_to\_target}=2925; the mean final EMA-swapped validation loss was 3.266926, giving $(3.28-\mu)\sqrt{2}=0.01849$ under the Track 3 rule. The same PR reports the $\beta_{\mathrm{target}}=0.999$ comparison run \texttt{b4q13sgm}, which reached 2950 steps and a worse final validation loss, indicating that the longer terminal EMA window introduced too much lag.
...
SENPAI's prompts route reusable workflows through progressively disclosed skills, while the controller and typed tools own polling, GitHub state transitions, and bookkeeping. This is a deliberate control choice: high-frequency control-plane operations should have narrow, testable interfaces instead of being reconstructed from shell commands by each agent invocation.
...
\begin{center}
  ...
  \begin{tabular}{@{}p{0.18\textwidth}p{0.15\textwidth}p{0.57\textwidth}@{}}
    ...
    \texttt{bootstrap-target} & Operator & Creates the target programme contract and role-specific instruction overlays for a new research repository. \\
...
This section contains verbatim snapshots of the prompt and contract artifacts summarized in \Cref{app:agent-instructions}. The files are vendored into \texttt{paper/appendix\_sources/} so the runtime contracts can be inspected with the paper.
...
\subsection{Local TandemFoilSet-Balanced Program Contract}
