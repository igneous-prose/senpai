...
def write_csv(prs: Iterable[PullRequest]) -> None:
    with (SCRIPT_DIR / "willow_prs_classified.csv").open("w", newline="") as f:
...
def write_summary(prs: list[PullRequest]) -> None:
    ...
    lines.append("")
    (SCRIPT_DIR / "willow_sankey_summary.md").write_text("\n".join(lines))
...
def main() -> None:
    ...
    print(f"Wrote {SCRIPT_DIR / 'willow_prs_classified.csv'}")
    print(f"Wrote {SCRIPT_DIR / 'willow_sankey_summary.md'}")
