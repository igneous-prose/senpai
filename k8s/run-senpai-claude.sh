...
#
# Shared agent invocation for Senpai advisor/student entrypoints.
...
run_senpai_claude() {
    ...
    local runtime="${SENPAI_AGENT_RUNTIME:-claude}"

    if [ "$runtime" = "openhands" ]; then
    ...
    if [ "$runtime" != "claude" ]; then
        echo "ERROR: SENPAI_AGENT_RUNTIME must be 'claude' or 'openhands' (got '$runtime')" >&2
        return 2
    fi
